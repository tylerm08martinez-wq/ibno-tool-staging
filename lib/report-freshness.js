...
(function (root, factory) {
  ...
  // ── The ONE mode-of-INBOUND_DATE reader (issue #464) ────────────────────
  //
  // A "sort date" for the Inbound and Van Scans report is the MOST COMMON
  // normalized INBOUND_DATE across its data rows. Two implementations of that
  // used to exist — AddressCatcherSession.inboundSortDate (skipped rows with a
  // blank PKG_LABEL_XREF) and IbnoSession.modeInboundDate (counted every dated
  // row) — and they DISAGREED on a file mixing blank-tracking rows dated one
  // day with real rows dated another. Both now delegate here: this is the only
  // place the two SESSION readers compute the mode, so
  // IbnoSession.mergeEarlySort's records-facing primary/secondary decision
  // reads the same number the notes do.
  //
  // NOT the only INBOUND_DATE reader in the repo. lib/sort-day.js keeps its
  // own (SortDay.sortDayOf) for the sort-day store, and it deliberately
  // DIVERGES — IbnoRules.toIsoDate(46246) is '' where sortDayOf reads the same
  // rows as '2026-08-12'. Pre-existing and documented in sort-day.js's own
  // header; read it before assuming these can be collapsed into one.
  //
  // RECONCILED SEMANTICS: SKIP blank-PKG_LABEL_XREF rows (the Address
  // Catcher's behaviour) — a row with no tracking id is not a package, so it
  // does not get a vote on which day the report is. Settled by MEASUREMENT,
  // not preference: across every real export in reports/source-files/ and
  // inbox/ carrying a PKG_LABEL_XREF header (8 files, 120,209 data rows) there
  // are ZERO blank-tracking rows, so the choice is unobservable on real data
  // and the safer reading wins. If that ever stops being true, this comment is
  // the thing to re-measure.
  //
  // Deliberately NOT the first data row's date: on a report pulled fresh
  // mid-sort the early-sort rows dated today can sort ahead of the bulk of
  // yesterday's rows, so "first row" would misidentify the file's sort date.
  // Ties break by FIRST OCCURRENCE.
  //
  // Takes the report's DATA rows plus a column getter, NOT a whole file — the
  // two callers slice off their SSRS preamble differently (the Address Catcher
  // via InboundScans.findHeaderIndex, the IBNO Coder via its own sliceReport,
  // which additionally requires a PKG_LABEL_XREF + INBOUND_DATE header), and
  // consolidating the mode must not quietly change either one's idea of where
  // the data starts.
  //
  //   dataRows  the rows BELOW the header row
  //   get       a lib/csv.js columnGetter bound to that header row
  //   opts.toIsoDate  the date normalizer. Optional: defaults to
  //                   IbnoRules.toIsoDate, resolved LAZILY (never at module
  //                   evaluation) so this file may be <script src>'d in any
  //                   order relative to lib/ibno-rules.js. The Address Catcher
  //                   passes its OWN normalizer, which returns '' when
  //                   IbnoRules isn't loaded — IbnoRules is optional there and
  //                   consolidating must not make it mandatory.
  //
  // Returns 'YYYY-MM-DD', or '' when no row yields a parseable date. '' is
  // "unknown", never a usable key — callers must treat it as such.
  function modeInboundDate(dataRows, get, opts) {
    if (!Array.isArray(dataRows) || dataRows.length === 0) return '';
    if (typeof get !== 'function') return '';
    const o = opts || {};
    const toIso = typeof o.toIsoDate === 'function' ? o.toIsoDate : defaultToIsoDate;
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (!get(row, 'PKG_LABEL_XREF')) continue; // no tracking id -> not a package
      const iso = toIso(get(row, 'INBOUND_DATE'));
      if (!iso) continue;
      if (counts[iso] === undefined) { counts[iso] = 0; order.push(iso); }
      counts[iso] += 1;
    }
    ...
    for (let i = 0; i < order.length; i++) {
      if (counts[order[i]] > bestCount) { bestCount = counts[order[i]]; best = order[i]; }
    }
    return best;
  }

  // The default date normalizer: the IBNO Coder's own toIsoDate. Resolved on
  // every call (cheap: a global read or a warm require) so neither <script>
  // order nor a late-loaded lib/ibno-rules.js can freeze a missing dependency
  // in. Returns '' when it genuinely isn't available, which reads through as
  // "no parseable date" rather than throwing.
  function defaultToIsoDate(raw) {
    const rules = (root && root.IbnoRules) ||
      (typeof require === 'function' ? require('./ibno-rules') : null);
    if (!rules || typeof rules.toIsoDate !== 'function') return '';
    return rules.toIsoDate(raw);
  }

  return {
    DAY_MS: DAY_MS,
    modeInboundDate: modeInboundDate,
