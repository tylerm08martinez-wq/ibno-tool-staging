...
(function (root, factory) {
  ...
  // listDir(token, coords) -> the JSON files directly in coords.path, as
  // [{ name, path, sha }]. (#913)
  //
  // The 999 Desk keeps ONE FILE PER WRITER (areas/<slug>.json), and a device
  // does not know who else is on shift — so the reader discovers the other
  // desks rather than being told. Every other call in this module addresses one
  // known path.
  //
  // A MISSING DIRECTORY IS EMPTY, NOT AN ERROR: on the first morning nobody has
  // published yet, and that is a normal state.
  //
  // AN AUTH FAILURE IS NOT EMPTY, AND THAT DISTINCTION IS THE POINT. An expired
  // PAT is the silent failure #912 warns about — the desk simply stops
  // publishing, which on a desk unstaffed two weekdays a week is
  // indistinguishable from a quiet day. Folding 401/403 into [] here would bury
  // it below anything a staleness alarm (#915) could ever detect.
  async function listDir(token, coords) {
    const repo = repoCoords(coords);
    const res = await root.fetch(apiUrl(repo) + '?ref=' + repo.branch + '&t=' + Date.now(), {
      headers: { Authorization: 'Bearer ' + token, Accept: 'application/vnd.github+json' },
    });
    ...
    return json
      .filter(function (f) { return f && f.type === 'file' && /\.json$/i.test(String(f.name || '')); })
      .map(function (f) { return { name: String(f.name), path: String(f.path), sha: f.sha || null }; });
  }
