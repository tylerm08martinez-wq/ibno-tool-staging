...
//
// ─── AND THE DOUBT TRAVELS WITH THE AREA (#1051, VERSION 2) ─────────────────
//
// VERSION 1 shipped `{ area, at }` and nothing else, so an area typed on the
// Post Sort lane — which `ActualArea.setEarly` stamps `early: true`, and which
// prints on the PUBLISHER's own barcode card WITH the #642 "UNCONFIRMED — Post
// Sort early guess, not yet scanned" line — arrived on the other desk as a
// plain `remote` entry and printed on paper with no line at all. Tyler,
// 2026-09-17: "The admins only use the post sort", so that was EVERY remote
// area in both directions, not an edge case. The same physical assignment read
// caveated on one desk and confident on the other.
//
// So the envelope carries one more key, and the direction of its default is
// the whole design:
//
//   areas: { "<label>": { area: '140', at: '2026-09-17', confirmed: true } }
//
// `confirmed: true` MEANS "the publisher answered this against the report this
// tool actually runs" — it is written for a `typed`/`auto` entry and OMITTED
// for an `early` one. THE MARKER IS ON THE CONFIRMED SIDE ON PURPOSE. An
// envelope that does not say — a VERSION 1 file from a device nobody has
// updated, a hand-placed file, a row whose key got lost — then reads as
// UNCONFIRMED by construction, with no branch anywhere having to remember to
// fail closed. The costs are not symmetric: wrongly marking a confirmed area
// costs a redundant caveat on paper, and wrongly omitting the caveat costs a
// package walked to a belt on the strength of a guess (#642).
//
// THE VERSION BUMP IS THE STATED COST, on ADR-0023 item 4's terms and for its
// stated reason: a new key an old build silently drops is exactly the shape
// that gets adopted as something it is not. `parse` READS 1 and 2 (a v1 file
// still applies, every row unconfirmed) and `build` always emits 2 — so a
// not-yet-updated device REFUSES this build's file outright, with reason
// 'version', whose pill already says UPDATE THIS DEVICE. That is loud and
// self-announcing; its areas dash there until it is updated, which is the
// pre-#1038 state, rather than printing an unmarked guess.
(function (root, factory) {
  ...
  // Bump whenever the payload changes shape (ADR-0023 item 4).
  //   1: { area, at } (#913).
  //   2: + per-entry `confirmed` (#1051). See the section on it in the header.
  const VERSION = 2;
  // Every version this build can READ. `build` always emits VERSION; `parse`
  // accepts anything in this list and refuses everything else out loud, the
  // same shape lib/day-export.js's SUPPORTED_VERSIONS has.
  //
  // 1 IS READABLE AND MUST STAY READABLE. Refusing it would take the other
  // desk's areas off the card entirely the moment one device is updated before
  // the other, for a file whose every row this build understands perfectly
  // well — it simply carries no `confirmed`, which is the unconfirmed reading,
  // which is the safe one. The break is deliberately ONE-WAY: new reads old,
  // old refuses new.
  ...
  function normalizeAreas(raw) {
    ...
    Object.keys(raw).forEach(function (label) {
      ...
      if (at) entry.at = at;
      // `confirmed` (#1051) rides on the same "only when set" rule `at` does,
      // and it is STRICTLY `=== true`: a truthy string or a 1 from a
      // hand-edited file is not a publisher's claim, and the fail-closed
      // direction costs a redundant caveat rather than a wrong stop.
      if (v.confirmed === true) entry.confirmed = true;
  ...
  function parse(raw) {
    ...
    if (READABLE_VERSIONS.indexOf(parsed.version) === -1) {
      return { ok: false, reason: 'version', error: 'unsupported version ' + JSON.stringify(parsed.version) + ' (this build reads ' + READABLE_VERSIONS.join(' and ') + ')' };
    }
    return {
      ok: true,
      envelope: {
        kind: KIND,
        // THE VERSION READ, not this build's. A v1 file stays a v1 file in
        // hand, so a caller reporting on the channel can say which build the
        // other desk is running — and re-stamping it 2 would make a file that
        // cannot say whether an area was confirmed look like one that chose
        // not to.
  ...
  function localAreas(store, opts) {
    ...
    Object.keys(src).forEach(function (label) {
      ...
      // THE DOUBT TRAVELS WITH THE AREA (#1051). `early` is the one provenance
      // this device prints WITH the #642 UNCONFIRMED line, so publishing it
      // without saying so shipped a caveated value as a confident one — see
      // the VERSION 2 section in the header for why the claim is made on the
      // CONFIRMED side and never on the doubtful one.
      //
      // Read off `early` and nothing else. A `remote` entry never reaches here
      // (the loop's stop condition, above), so what is left is typed, auto, or
      // a markerless legacy entry — and a markerless one is claimed CONFIRMED
      // deliberately: adoptLegacyTyped has already decided on this device that
      // a markerless area is hand-typed work, and telling the other desk
      // something different about the same entry would put two opinions about
      // one row on two screens.
      if (!e.early) entry.confirmed = true;
      out[label] = entry;
    });
    return out;
  }
  ...
  function applyTo(store, env, opts) {
    ...
    Object.keys(areas).forEach(function (label) {
      ...
      // `confirmed` (#1051) is handed to setRemote as it arrived, ABSENT AND
      // ALL. An entry that does not claim confirmation lands unconfirmed, so a
      // VERSION 1 envelope — every row of it — arrives carrying the doubt, and
      // the receiving card carries the #642 line. That is the fail-closed
      // direction: the cost is a redundant caveat, never a wrong stop.
      const r = ActualArea.setRemote(next, label, areas[label].area,
        { today: stamp, by: by, confirmed: areas[label].confirmed === true });
      next = r.store;
