...
(function (root, factory) {
  ...
  function restore(raw) {
    ...
    Object.keys(parsed).forEach(function (label) {
      ...
      if (typeof v === 'object' && v.remote) {
        entry.remote = true;
        // `remoteConfirmed` (#1051) — the other desk SAID this area was
        // answered against the report, rather than pre-filled on the Post Sort
        // lane. Carried on the same terms as `remote` and NESTED INSIDE IT on
        // purpose: the flag qualifies a remote entry and means nothing without
        // one, so it can never exist alone to be misread by a reader that has
        // not heard of it.
        //
        // ABSENT IS THE UNCONFIRMED READING, and that is what makes this
        // fail-closed at the one boundary nobody controls: an area store
        // persisted by a pre-#1051 build holds `remote` with no claim beside
        // it, and it reads as unconfirmed after the upgrade — a redundant
        // caveat on paper rather than an unmarked guess.
        //
        // STRICTLY `=== true`, matching normalizeAreas() in lib/remote-areas.js
        // and setRemote() below, because restore() IS a trust boundary: the Day
        // Export merge (lib/day-export.js) runs it over a FOREIGN file. A
        // truthy `1` or `"yes"` from a hand-edited or malformed export is not a
        // publisher's claim, and reading one as CONFIRMED would print the area
        // plain — the exact fail-open this design exists to prevent. Costs
        // nothing internally: setRemote only ever writes a literal `true`.
        if (v.remoteConfirmed === true) entry.remoteConfirmed = true;
      }
  ...
  function setEarly(store, label, area, opts) {
    ...
    // it. And the barcode card's 30px line changes hands: areaProvenanceOf now
    // answers `early` for this entry instead of `remote`, so the card prints it
    // WITH the #642 UNCONFIRMED mark, which is right — it is a Post Sort value
    // typed against no scan, whoever first said the number.
    //
    // THE SECOND CONSEQUENCE USED TO BE THAT IT PRINTED AT ALL (#1038, PR
    // #1047). areaProvenanceOf allowlisted typed/auto/early only, the card's
    // line was "restricted to an area decided ON THIS DEVICE", and a
    // still-`remote` entry DASHED — so his agreement is what earned the line.
    // `remote` is on PRINTABLE_PROVENANCE now and both versions print. Do not
    // read the old sentence as a reason to keep `remote` on agreement: the
    // reason it GOES is the withdrawal pass above, not the card.
    //
    ...
    if (!before || before.area !== value) {
      ...
      delete kept.remote;
      // `remoteConfirmed` GOES WITH `remote` (#1051). It qualifies a remote
      // entry and restore() will not carry it without one, so leaving it here
      // would hand the next reader a flag about a row that is no longer
      // remote — the entry is this device's `early` answer now, and its
      // unconfirmed-ness is `early`'s to state.
      delete kept.remoteConfirmed;
      next[key] = kept;
      changed.push(key);
    }
  ...
  // isRemoteConfirmed(store, label) -> did the OTHER DESK claim this area was
  // answered against the report, rather than pre-filled on the Post Sort lane
  // ahead of it? (#1051, RemoteAreas VERSION 2.)
  //
  // A SECOND AXIS, NOT A FIFTH PROVENANCE FLAG. `remote` answers "where did
  // this come from"; this answers "how sure was the desk it came from". They
  // are genuinely independent — the other desk's area may itself have been
  // typed early or not, which is the very thing #913 said this device could
  // not tell and this flag is how it now can — and ADR-0022's rule is that a
  // consumer asking a different question gets its own predicate.
  //
  // IT IS A POSITIVE READER FOR A POSITIVE MARKER, exactly as isEarly and
  // isRemote are, and the marker is on the CONFIRMED side deliberately. False
  // is therefore the answer for a remote entry that says nothing — a v1
  // envelope, a store persisted before #1051, a hand-placed file — and the
  // card carries the #642 line for all of them. That is not reconstructing a
  // provenance from the absence of others (#722's mistake): `remote` is still
  // read off its own flag, and this asks a separate question whose safe pole
  // is the silent one.
  //
  // FALSE FOR EVERYTHING THAT IS NOT REMOTE AT ALL, and callers must not read
  // that as doubt: a `typed` entry is not an unconfirmed remote one. Ask
  // isRemote first — areaProvenanceOf does.
  function isRemoteConfirmed(store, label) {
    const e = store && store[String(label)];
    return !!(e && e.remote && e.remoteConfirmed);
  }

  // setRemote(store, label, area, opts) -> { store, changed }
  //
  // Applies an area the OTHER desk typed, arriving over the 999 Desk live loop
  // (#913). Writes { area, date, remote: true, remoteConfirmed?, by }.
  //
  // `opts.confirmed` (#1051) IS THE PUBLISHER'S CLAIM ABOUT THEIR OWN AREA,
  // carried on RemoteAreas VERSION 2 and written here as `remoteConfirmed`.
  // Absent means unconfirmed, and absent is what every VERSION 1 envelope and
  // every pre-#1051 persisted entry says — see isRemoteConfirmed above for why
  // the marker sits on the confirmed side. It gates nothing: this lane's
  // overwrite refusal is exactly what it was.
  //
  ...
  //     Pinned by tests/actual-area-remote.test.js so it stays a decision
  //     rather than an accident of which flag happened to be checked.
  //
  // WHAT NO LONGER FOLLOWS: THE BARCODE CARD PRINTS IT (#1038, 2026-09-17).
  // Until then areaProvenanceOf() allowlisted typed/auto/early only, so a
  // remote area DASHED on the card — "restricted to an area decided on this
  // device". That reading did not survive contact with the floor: under the 999
  // Desk two people work one list, the row on screen showed their area, and the
  // paper the admin walks the dock with showed a dash for the same package.
  // areaProvenanceOf() now answers 'remote' (its own answer, never folded into
  // 'typed') and IbnoBarcodeSheet.PRINTABLE_PROVENANCE admits it. That is a
  // permission to PRINT and nothing more: the propagation refusal above is
  // untouched, because printing the card a person decided is not the same thing
  // as spreading their answer onto packages they never saw.
  function setRemote(store, label, area, opts) {
    ...
    const stamp = o.today ? String(o.today) : '';
    const by = normalizeBy(o.by);
    // `opts.confirmed` (#1051) — did the PUBLISHER say this area was answered
    // against the report? Strictly `=== true`: the caller is a wire format
    // (RemoteAreas VERSION 2), and everything it does not say must fall to the
    // unconfirmed side. It changes no rule about WHO MAY WRITE here —
    // oursToTouch below is untouched — only what the written entry says about
    // itself.
    ...
    //
    // IDENTICAL INCLUDES THE CONFIRMATION CLAIM (#1051). Without that term the
    // claim could only ever be written on the pull that first brought the
    // area, so the two moves that change it alone would both be swallowed: the
    // other desk updating from a VERSION 1 build to a VERSION 2 one mid-sort
    // (their unconfirmed-by-omission rows would never gain the claim), and a
    // pre-#1051 entry already sitting in this device's persisted store. The
    // area and the author agreeing is not the same as the whole answer
    // agreeing.
    if (before && before.area === value && normalizeBy(before.by) === by &&
        !!before.remoteConfirmed === confirmed) {
      return { store: next, changed: [] };
    }

    const e = { area: value, date: stamp, remote: true };
    if (confirmed) e.remoteConfirmed = true;
    if (by) e.by = by;
    next[key] = e;
    changed.push(key);
    return { store: next, changed: changed };
  ...
  return {
    ...
    isEarly: isEarly,
    isRemote: isRemote,
    isRemoteConfirmed: isRemoteConfirmed,
    byOf: byOf,
    otherBy: otherBy,
